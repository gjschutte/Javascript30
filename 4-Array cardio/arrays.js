...
function is1950 (value) {
    return (value.year >= 1500 && value.year < 1600);
}

const fifteenInventors = inventors.filter(is1950);
console.log("Filter the list of inventors for those who were born in the 1500's");
console.log(fifteenInventors);

// Array.prototype.map()
// 2. Give us an array of the inventors first and last names

const nameInventors = inventors.map (inventor => `${inventor.first} ${inventor.last}`)
...
let initialValue = 0;
let sumAges = inventors.reduce((accumulator, currentValue) => 
...
let sortedAge = inventors.sort((a, b) => 
...
let sortFirst = people.sort((a, b) => {
    let splitNameA = a.split (", ");
    let splitNameB = b.split (", ");
    return splitNameA[1] > splitNameB[1] ? 1 : -1});

console.log("Sort the people alphabetically by first name");
console.table(sortFirst);
